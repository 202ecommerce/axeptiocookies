...
import qs from 'qs';
import type {CreatableConfiguration, EditableConfiguration} from "../types/types.ts";
...
export default class ClientAPI {
  ...
  private getProjectIdCancelToken: Function | undefined;

  async getCookiesByProjectId(idProject: string) {
    try {
      const self = this;
  ...
  async createConfiguration(configuration: CreatableConfiguration) {
    try {
      const url = window.axeptiocookies.links.ajax;
  ...
  async editConfiguration(configuration: EditableConfiguration) {
    try {
      const url = window.axeptiocookies.links.ajax;
